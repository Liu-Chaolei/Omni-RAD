"""
trainv1_variable3_policynet_step.py — End-to-end training of TimestepPolicyNet.

Based on trainv1_variable3_debug_step.py. Key changes:
  1. Uses StableCodec_variable2_step_policy (TimestepPolicyNet replaces DynamicTimestepModule).
  2. forward() returns (output_image, RateLossOutput, T_pred).
  3. Two-phase training:
     Phase 1: freeze main model, only train PolicyNet (lr=1e-4)
     Phase 2: joint finetune PolicyNet + LoRA + FiLM (PolicyNet lr=1e-4, rest lr=1e-5)
  4. Logs Train/T_pred_mean, Train/T_pred_std, Train/T_snr_mean, corr(T_pred, T_snr).
  5. PolicyNet is part of state_dict_codec (saved/loaded with codec).
"""
...
from StableCodec_variable2_step_policy import StableCodec
...
def sample_lambda(B: int, lambda_min: float, lambda_max: float, device) -> torch.Tensor:
    """Sample B λ values log-uniformly in [lambda_min, lambda_max].

    Log-uniform ensures equal density of RD-curve coverage in log-space, matching
    the exponential mapping Λ = θ·exp(τ·L) used by I2C.  Each image in the
    batch may receive a different λ, maximising the effective number of distinct
    rate-distortion targets seen per training step.
    """
...
def configure_optimizers(net, net_disc, train_config, world_size, stage, rank, save_path, accelerator, structure_logger=None, policy_phase=1):
    """Configure optimizers with two-phase PolicyNet training support.

    Phase 1: Only PolicyNet params are trainable (single param group, lr=policy_lr).
    Phase 2: PolicyNet (lr=policy_lr) + rest of trainable params (lr=base_lr).
    """
    policy_config = train_config.get('policy_net', {})
    policy_lr = float(policy_config.get('lr', 1e-4))
    base_lr = float(train_config.get('learning_rate', 1e-5))

    if train_config.get('scale_lr', False):
        scale_factor = (
            train_config['gradient_accumulation_steps']
            * train_config.get('batch_size', 1) * world_size
        )
        policy_lr *= scale_factor
        base_lr *= scale_factor

    params_dict = dict(net.named_parameters())

    # Identify PolicyNet parameter names
    policy_param_names = {
        n for n, p in net.named_parameters()
        if "timestep_policy" in n and not n.endswith(".quantiles") and p.requires_grad
    }
    # Non-policy trainable parameters (excluding entropy bottleneck quantiles)
    main_param_names = {
        n for n, p in net.named_parameters()
        if "timestep_policy" not in n
        and not n.endswith(".quantiles")
        and p.requires_grad
    }
    # Aux parameters (entropy bottleneck quantiles)
    aux_parameters = {
        n for n, p in net.named_parameters()
        if n.endswith(".quantiles") and p.requires_grad
    }

    assert len((policy_param_names | main_param_names) & aux_parameters) == 0
    ...
    if accelerator.is_main_process:
        ...
        structure_logger.info(f"[Phase {policy_phase}] PolicyNet Parameters ({len(policy_param_names)}): {sorted(list(policy_param_names))}")
        structure_logger.info(f"[Phase {policy_phase}] Main Parameters ({len(main_param_names)}): {sorted(list(main_param_names))}")
        structure_logger.info(f"[Phase {policy_phase}] Aux Parameters ({len(aux_parameters)}): {sorted(list(aux_parameters))}")

    # Build param groups
    param_groups = []
    if policy_param_names:
        param_groups.append({
            'params': [params_dict[n] for n in sorted(policy_param_names)],
            'lr': policy_lr,
            'name': 'policy_net',
        })
    if policy_phase == 2 and main_param_names:
        param_groups.append({
            'params': [params_dict[n] for n in sorted(main_param_names)],
            'lr': base_lr,
            'name': 'main',
        })

    # Fallback: if no trainable params found (shouldn't happen), use empty group
    if not param_groups:
        param_groups.append({'params': [], 'lr': base_lr, 'name': 'empty'})
    ...
    if aux_parameters:
        ...
        # Phase 1 freezes everything including quantiles — create a no-op optimizer
        _dummy = torch.nn.Parameter(torch.zeros(1), requires_grad=True)
        aux_optimizer = optim.AdamW(
            [_dummy],
            lr=train_config['aux_learning_rate'],
        )
...
def log_validation(step, val_dataloader, model, criterion, iqa_psnr,
    ...
    # For validation, use the reference λ (lambda_ref from config) for a fair comparison.
    ...
    with torch.no_grad():
        ...
        for d in val_dataloader:
            ...
            # Validation: fixed reference λ for reproducible metrics
            lmbda_val = torch.full((B,), lambda_ref, dtype=torch.float32, device=device)
            x_hat, RateLossOutput, T_star, _ = model(d, pos_tag_prompt, H, W, lmbda_val)
    ...
    if total_count > 0:
        ...
        avg_t_star = (metrics[5] / total_count).item()
    else:
        avg_loss = avg_bpp = avg_mse = avg_lpips = avg_psnr = avg_t_star = 0.0
...
def load_train_objs(train_dataloader, stage, train_config, val_config,
                    world_size, device, rank, save_path, accelerator, policy_phase=1):
    # Variable-rate: StableCodec no longer takes a fixed lmbda
    if accelerator.is_main_process:
        structure_logger = logger_setup(log_file_name='structure.log', log_file_folder_name=os.path.join(save_path, 'logs'))
    else:
    ...
    net = StableCodec(
        ...
        config=train_config['model'],   # must contain lambda_min, lambda_max
    ...
    # ---- Oracle PolicyNet warm-start ----
    policy_config = train_config.get('policy_net', {})
    oracle_init_path = policy_config.get('oracle_init_path', None)
    if oracle_init_path and os.path.isfile(oracle_init_path):
        oracle_ckpt = torch.load(oracle_init_path, map_location="cpu")
        oracle_sd = oracle_ckpt.get("state_dict", oracle_ckpt)
        # Map oracle keys into codec.timestep_policy namespace
        policy_sd = {}
        for k, v in oracle_sd.items():
            # oracle checkpoint keys are bare (e.g. "net.0.weight")
            # codec expects them under timestep_policy prefix
            policy_sd[k] = v
        missing, unexpected = net.codec.timestep_policy.load_state_dict(policy_sd, strict=False)
        if accelerator.is_main_process:
            if structure_logger:
                structure_logger.info(f"[Oracle Init] Loaded PolicyNet from: {oracle_init_path}")
                if missing:
                    structure_logger.info(f"[Oracle Init] Missing keys: {missing}")
                if unexpected:
                    structure_logger.info(f"[Oracle Init] Unexpected keys: {unexpected}")
            else:
                print(f"[Oracle Init] Loaded PolicyNet from: {oracle_init_path}")

    # Apply phase-specific freeze/unfreeze before building optimizer
    if policy_phase == 1:
        net.set_train_phase1()
    else:
        net.set_train_phase2()

    optimizer, aux_optimizer, disc_optimizer = configure_optimizers(
        net, net_disc, train_config, world_size, stage, rank, save_path, accelerator,
        structure_logger=structure_logger, policy_phase=policy_phase)
...
def main(stage, base_config, train_config, val_config, MASTER_PORT, experiment_name):
    ...
    # -------------------- Two-phase PolicyNet training --------------------
    policy_config = train_config.get('policy_net', {})
    policy_phase1_steps = int(policy_config.get('phase1_steps', 20000))
    initial_policy_phase = int(policy_config.get('phase', 1))  # start phase from config
    ...
    net, net_disc, optimizer, aux_optimizer, disc_optimizer, lr_scheduler, disc_lr_scheduler = \
        load_train_objs(train_dataloader, stage, train_config, val_config,
                        ...
                        policy_phase=initial_policy_phase)
    ...
    if train_config['model'].get("resume_train"):
        ...
        # Restore policy phase from checkpoint if available
        if "policy_phase" in sd:
            resumed_phase = int(sd["policy_phase"])
            if resumed_phase != initial_policy_phase:
                print(f"[INFO] Checkpoint was saved at policy_phase={resumed_phase}, "
                      f"overriding config phase={initial_policy_phase}")
                initial_policy_phase = resumed_phase
                # Re-apply the correct freeze/unfreeze for the resumed phase
                if resumed_phase == 2:
                    net.set_train_phase2()
                else:
                    net.set_train_phase1()
                # Rebuild optimizer for the correct phase
                optimizer, aux_optimizer, _ = configure_optimizers(
                    net, net_disc, train_config, world_size, stage, rank, save_path,
                    accelerator, policy_phase=resumed_phase)
        try:
    ...
    # Reference λ: used for distortion-loss rescaling and validation
    lambda_ref = float(train_config['model'].get('lambda_ref', train_config.get('lmbda', 1.0)))
    # Fallback defaults are intentionally wide to avoid clamp saturation in LambdaFiLMEmbed.
    # Always set model.lambda_min / model.lambda_max explicitly in the YAML config.
    lambda_min = float(train_config['model'].get('lambda_min', 0.1))
    lambda_max = float(train_config['model'].get('lambda_max', 128.0))
    # Sampling range can be narrower than model range (FiLM embedding uses lambda_min/lambda_max)
    lambda_sample_max = float(train_config['model'].get('lambda_sample_max', lambda_max))

    # -------------------- rho schedule (CFT / gated fusion) --------------------
    rho_start_step = int(train_config.get('rho_start_step', 0))
    rho_warmup_steps = int(train_config.get('rho_warmup_steps', 0))
    rho_final = float(train_config.get('rho_final', 1.0))
    ...
    metric_names = ['Train/loss','Train/mse_loss','Train/lpips','Train/bpp',
                    ...
                    'Train/T_pred_mean','Train/T_pred_std']

    # -------------------- λ-BPP-T diagnostics --------------------
    diag_every    = int(train_config.get('diag_every', 500))
    diag_num_bins = int(train_config.get('diag_num_bins', 6))
    diag_buffer   = {"lambda": [], "bpp": [], "dist": [], "lambda_dist": [], "t_pred": []}
    ...
    def _log_diag_and_clear(logger, step):
        lam  = np.asarray(diag_buffer["lambda"], dtype=np.float64)
        bpp  = np.asarray(diag_buffer["bpp"],    dtype=np.float64)
        dist = np.asarray(diag_buffer["dist"],   dtype=np.float64)
        lamdist = np.asarray(diag_buffer["lambda_dist"], dtype=np.float64)
        tpred = np.asarray(diag_buffer["t_pred"], dtype=np.float64)
        if lam.size == 0:
            return
        loglam = np.log(np.clip(lam, 1e-12, None))
        msg = (f"[DIAG step={step}] n={lam.size} "
               f"corr(logλ,bpp)={_safe_corr(loglam, bpp):.4f} "
               f"corr(logλ,dist)={_safe_corr(loglam, dist):.4f} "
               f"corr(logλ,λ·dist)={_safe_corr(loglam, lamdist):.4f} "
               f"corr(logλ,T_pred)={_safe_corr(loglam, tpred):.4f} "
               f"mean_bpp={bpp.mean():.6f} "
               f"range_bpp=({bpp.min():.6f},{bpp.max():.6f}) "
               f"mean_T_pred={tpred.mean():.1f} std_T_pred={tpred.std():.1f}")
        print(msg)
        if logger is not None:
            logger.info(msg)
        lo, hi = loglam.min(), loglam.max()
        if hi - lo > 1e-12 and lam.size >= diag_num_bins:
            edges = np.linspace(lo, hi, diag_num_bins + 1)
            idx = np.digitize(loglam, edges[1:-1], right=False)
            for bi in range(diag_num_bins):
                m = (idx == bi)
                if not np.any(m):
                    continue
                bmsg = (f"[DIAG-BIN step={step}] bin={bi}/{diag_num_bins-1} "
                        f"logλ∈[{edges[bi]:.3f},{edges[bi+1]:.3f}] "
                        f"n={m.sum()} "
                        f"mean_λ={lam[m].mean():.4f} "
                        f"mean_bpp={bpp[m].mean():.6f} "
                        f"mean_dist={dist[m].mean():.6f} "
                        f"mean_λdist={lamdist[m].mean():.6f} "
                        f"mean_T_pred={tpred[m].mean():.1f}")
                print(bmsg)
                if logger is not None:
                    logger.info(bmsg)
    ...
    # Explicitly zero gradients before entering the loop to avoid stale grads after resume
    optimizer.zero_grad(set_to_none=True)
    aux_optimizer.zero_grad(set_to_none=True)
    if disc_optimizer:
        disc_optimizer.zero_grad(set_to_none=True)

    # Track current policy phase for phase transition
    current_policy_phase = initial_policy_phase
    # If resuming into phase 2, mark transition as already done
    phase_transitioned = (initial_policy_phase >= 2)

    while global_step < train_config['max_train_steps']:
        print(f"Learning rate: {optimizer.param_groups[0]['lr']}")
        if accelerator.is_main_process:
            train_logger.info(f"Learning rate: {optimizer.param_groups[0]['lr']}")
            train_logger.info(f"PolicyNet phase: {current_policy_phase}")
        ...
        for i, d in enumerate(train_dataloader):
            ...
            # --------------------------------------------------------
            if (current_policy_phase == 1
                    and not phase_transitioned
                    and global_step >= policy_phase1_steps):
                if accelerator.is_main_process:
                    train_logger.info(
                        f"[PHASE TRANSITION] step={global_step}: "
                        f"Phase 1 → Phase 2 (unfreezing UNet LoRA + FiLM + unet_lora_proj)")
                    # Save phase1 checkpoint before transition
                    _save_ckpt(net, ema_net, optimizer, aux_optimizer, lr_scheduler,
                               epoch, global_step, save_path, train_config, accelerator,
                               "checkpoint-phase1-final.pth.tar",
                               net_disc=net_disc, disc_optimizer=disc_optimizer,
                               disc_lr_scheduler=disc_lr_scheduler)

                # Switch model to phase 2 (unfreeze more params)
                unwrap_model(accelerator, net).set_train_phase2()

                # Rebuild optimizer with new param groups for phase 2
                policy_lr = float(policy_config.get('lr', 1e-4))
                base_lr = float(train_config.get('learning_rate', 1e-5))
                unwrapped_net = unwrap_model(accelerator, net)
                params_dict = dict(unwrapped_net.named_parameters())

                policy_param_names = {
                    n for n, p in unwrapped_net.named_parameters()
                    if "timestep_policy" in n and not n.endswith(".quantiles") and p.requires_grad
                }
                main_param_names = {
                    n for n, p in unwrapped_net.named_parameters()
                ...
                # Rebuild LR scheduler for remaining steps
                remaining_steps = train_config['max_train_steps'] - global_step
                num_warmup_p2 = int(policy_config.get('phase2_warmup_steps', 500)) * world_size
                lr_scheduler = get_scheduler(
                    train_config['lr_scheduler'], optimizer=optimizer,
                    num_warmup_steps=num_warmup_p2,
                    num_training_steps=remaining_steps * world_size,
                ...
                current_policy_phase = 2
                phase_transitioned = True

                if accelerator.is_main_process:
                    train_logger.info(
                        f"[PHASE 2] PolicyNet params: {len(policy_param_names)}, "
                        f"Main params: {len(main_param_names)}, "
                        f"PolicyNet lr={policy_lr}, Main lr={base_lr}")

            # --------------------------------------------------------
            # Set train mode (phase-aware)
            # --------------------------------------------------------
            if current_policy_phase == 1:
                unwrap_model(accelerator, net).set_train_phase1()
            else:
                unwrap_model(accelerator, net).set_train_phase2()
            if stage == 2:
                net_disc.train()
            ...
            with accelerator.accumulate(net):
                ...
                # --------------------------------------------------------
                # Sample per-image λ  (log-uniform in [lambda_min, lambda_sample_max])
                # --------------------------------------------------------
                lmbda = sample_lambda(B, lambda_min, lambda_sample_max, device)

                x_hat, RateLossOutput, T_pred, policy_info = net(d, pos_tag_prompt,
                                                    train_config['patch_size'][0],
                                                    train_config['patch_size'][1],
                                                    lmbda=lmbda)
                d = d.detach().float()

                # Distortion loss from criterion (computed with fixed lambda_ref)
                out_criterion = criterion(x_hat, d)
                ...
                #
                # TargetRateModule.rate_loss = mean(bpp_i)  (unweighted)
                # Distortion is divided by per-image λ.
                # --------------------------------------------------------
                ...
                with torch.no_grad():
                    diag_buffer["lambda"].extend(lmbda.detach().cpu().numpy().tolist())
                    ...
                    diag_buffer["lambda_dist"].extend((lmbda * per_image_dist).detach().cpu().numpy().tolist())
                    diag_buffer["t_pred"].extend(T_pred.detach().cpu().numpy().tolist())
                ...
                # PolicyNet delta_T monitoring
                T_snr = policy_info["T_snr"].detach()
                delta_T = T_pred - T_snr
                ...
                current_metrics = {
                    ...
                    'Train/loss_D':      0.0,
                    'Train/aux_loss':    0.0,
                    # PolicyNet timestep monitoring
                    'Train/T_pred_mean': T_pred.mean().item(),
                    'Train/T_pred_std':  T_pred.std().item() if T_pred.numel() > 1 else 0.0,
                    'Train/T_snr_mean':  T_snr.mean().item(),
            ...
            if accelerator.sync_gradients:
                ...
                if global_step < 20000 or global_step % train_config['log_every'] == 0:
                    ...
                    if train_config['wandb']:
                        ...
                        # Log policy phase to wandb
                        wandb_utils.log({'train/policy_phase': current_policy_phase}, step=global_step)
...
def _save_ckpt(net, ema_net, optimizer, aux_optimizer, lr_scheduler,
    ...
    """Build and save a checkpoint dict including variable-rate module weights."""
    ...
    ckpt_dict = {
        ...
        # Variable-rate λ-FiLM projection (Injection Point 3)
        "state_dict_lora_proj": unwrapped.unet_lora_proj.state_dict(),
        "optimizer":        optimizer.state_dict(),
        "aux_optimizer":    aux_optimizer.state_dict(),
        "lr_scheduler":     lr_scheduler.state_dict(),
        # PolicyNet phase tracking for resume
        "policy_phase":     getattr(unwrapped, '_current_policy_phase', 1),
    }
    if train_config['save_ema']:
...
# CUDA_VISIBLE_DEVICES=5 accelerate launch --num_processes 1 src/trainv1_variable2.py --stage 2 --MASTER_PORT 12356 --experiment_name ft2_step999_new
