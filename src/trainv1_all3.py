...
from my_utils.utils import RateDistortionLoss_fusion, logger_setup, get_raw_state_dict
...
def configure_optimizers(net, net_disc, train_config, world_size, stage, rank, save_path, accelerator, structure_logger=None):
    """
    Build the main optimizer (all trainable non-quantile params) and the
    auxiliary optimizer (entropy-bottleneck quantile params).

    The discriminator has been removed from the 4-stage training scheme.
    """
    ...
    assert len(parameters & aux_parameters) == 0, \
        "Parameter sets must not overlap!"
...
def save_model(
    ...
    ckpt_dict = {
            ...
            "state_dict_cft": unwarp_net.cft_blocks.state_dict(),
            "optimizer": optimizer.state_dict(),
            "aux_optimizer": aux_optimizer.state_dict(),
            "lr_scheduler": lr_scheduler.state_dict(),
            "state_dict_lambda_w": dict(unwarp_net.lambda_adaptive_w.state_dict())
    }
...
def load_train_objs(train_dataloader, stage, train_config, val_config, world_size, device, rank, save_path, accelerator):
    ...
    # Configure trainable parameters and cft_w for this stage.
    # This MUST happen before configure_optimizers so that the optimizer
    # only receives the parameters that are actually active.
    net.set_train(stage)

    # ── Optional: memory / speed flags ────────────────────────────────
    if train_config['enable_xformers_memory_efficient_attention']:
        if is_xformers_available():
            net.unet.enable_xformers_memory_efficient_attention()
        else:
            raise ValueError(
                "xformers is not available. Install it with `pip install xformers`."
            )
    if train_config['gradient_checkpointing']:
        net.unet.enable_gradient_checkpointing()
        if hasattr(net.vae, 'enable_gradient_checkpointing'):
    ...
    num_warmup_steps_for_scheduler = (
        train_config['lr_warmup_steps'] * world_size
    )
    if train_config['max_train_steps'] is None:
        len_after_sharding = math.ceil(len(train_dataloader) / world_size)
        num_update_steps_per_epoch = math.ceil(
            len_after_sharding / train_config['gradient_accumulation_steps']
        )
        ...
        num_training_steps_for_scheduler = (
            train_config['max_train_steps'] * world_size
        )
...
def main(
    ...
    net, net_disc, optimizer, aux_optimizer, disc_optimizer, lr_scheduler, disc_lr_scheduler = load_train_objs(
        train_dataloader, stage, train_config, val_config,
    ...
    if train_config['model']["resume_train"]:
        ...
        if (stage == 2 or stage == 4) and "state_dict_disc" in sd and "disc_optimizer" in sd and "disc_lr_scheduler" in sd:
    ...
    if stage == 2 or stage == 4:
        net, optimizer, aux_optimizer, train_dataloader, val_dataloader, net_disc, disc_optimizer = accelerator.prepare(
            net, optimizer, aux_optimizer, train_dataloader, val_dataloader, net_disc, disc_optimizer
        )
    else:
        net, optimizer, aux_optimizer, train_dataloader, val_dataloader = accelerator.prepare(
            net, optimizer, aux_optimizer, train_dataloader, val_dataloader
        )
    net.to(accelerator.device, dtype=weight_dtype)


    criterion = RateDistortionLoss_fusion(
        lmbda=train_config['lmbda'],
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % val_config['validation_steps'] == 0 or (val_config['val_first'] and global_step == 1):
                    ...
                    val_loss, val_bpp, val_mse, val_lpips, val_psnr = log_validation(
                        global_step, val_dataloader, net, criterion,
                        iqa_psnr, val_config['save_val'], val_config['save_steps'],
                    ...
                    if accelerator.is_main_process:
                        ...
                        if (train_config['save_best'] and val_loss <= best_loss and global_step > train_config['min_steps_for_best']):
                            ...
                            val_logger.info(
                                f'New best model saved at step {global_step} with LOSS: {best_loss:.3f}'
                            )
                    start_time = time.time()
        epoch += 1
    accelerator.end_training()

# CUDA_VISIBLE_DEVICES=5 accelerate launch --num_processes 1 src/trainv1_all3.py --stage 2 --MASTER_PORT 12356 --experiment_name ft2_step999_new
