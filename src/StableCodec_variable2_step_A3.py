"""
StableCodec_variable2_step_A3.py — A3 variant: dual-gate mechanism with separate
spatial gates for UNet repair residual and AuxDecoder structural residual.

A3 changes vs. A0 (StableCodec_variable2_step.py):
  • Import: uses latent_codec_variable2_step_A3 (5-tuple forward, 4-tuple decompress).
  • UncertaintyGateModule: spatial gate with learnable α/β.
    Init α=0, β=4 → M≈0.98 at start (identity w.r.t. Stage I).
  • Two gate instances: gate_unet (controls delta_unet) and gate_aux (controls res1).
  • forward fusion (with rho transition):
      z_direct = x0_pred + res1  (Stage I formula)
      z_gated  = sample + M_unet*delta_unet + M_aux*res1  (dual-gated)
      z_hat    = (1-ρ)*z_direct + ρ*z_gated
    When rho=0: equivalent to Stage I. When rho=1, M_unet=M_aux=1: also equivalent.
  • decompress fusion: sample + M_unet*delta_unet + M_aux*res (rho=1 at inference).
  • save_model: additionally saves state_dict_gate_unet and state_dict_gate_aux.
  • Checkpoint loading: gracefully handles missing gate weights (warm-start from A0).
  • set_train: both gate modules' parameters are trainable.
...
from latent_codec_variable2_step_A3 import LatentCodec, LAMBDA_MIN, LAMBDA_MAX
import sys
sys.path.append("..")
...
class UncertaintyGateModule(nn.Module):
    """A3: Spatial uncertainty gate M ∈ [0,1]^{B×1×H/8×W/8}.

    Derives a per-pixel gate from the entropy model's scale map.
    Init: α=0, β=4 → sigmoid(4)≈0.98 → M≈1 at start (identity w.r.t. Stage I).
    """
    ...
    def forward(self, scales_all: torch.Tensor, target_hw: tuple) -> torch.Tensor:
        s = scales_all.mean(dim=1, keepdim=True)
        s = (s - s.mean(dim=[2, 3], keepdim=True)) / (s.std(dim=[2, 3], keepdim=True) + 1e-6)
        s_up = F.interpolate(s, size=target_hw, mode='bilinear', align_corners=False)
        return torch.sigmoid(self.alpha * s_up + self.beta)
...
class StableCodec(torch.nn.Module):
    def __init__(self, sd_path=None, config=None, logger=None, stage=None):
        ...
        # ----------------------------------------------------------------
        from latent_codec_variable2_step_A3 import FILM_DIM
        ...
        # A3: Dual spatial uncertainty gate modules
        self.gate_unet = UncertaintyGateModule()
        self.gate_aux = UncertaintyGateModule()
        ...
        if config.get('codec_path') is not None:
            ...
            # A3: dual gate modules (may not exist when warm-starting from A0)
            if "state_dict_gate_unet" in sd:
                self.gate_unet.load_state_dict(sd["state_dict_gate_unet"])
                print("  -> Loaded gate_unet weights.")
            else:
                print("  -> No gate_unet weights in checkpoint; using init (alpha=0, beta=0).")
            if "state_dict_gate_aux" in sd:
                self.gate_aux.load_state_dict(sd["state_dict_gate_aux"])
                print("  -> Loaded gate_aux weights.")
            else:
                print("  -> No gate_aux weights in checkpoint; using init (alpha=0, beta=0).")
    ...
    def set_train(self):
        ...
        # A3: Dual gate modules
        for p in self.gate_unet.parameters():
            p.requires_grad = True
        for p in self.gate_aux.parameters():
            p.requires_grad = True
    ...
    def forward(self, x, pos_prompt, ori_h, ori_w, lmbda=None, rho=1.0):
        """
        Args:
            ...
            rho:        float in [0,1]. Transition from direct fusion (0) to
                        dual-gated fusion (1). Training loop schedules this.
        ...
        # A3: Dual-gated fusion — separate gates for UNet repair and AuxDecoder residual
        # z_direct = x0_pred + res1 (Stage I formula)
        # z_gated = sample + M_unet*delta_unet + M_aux*res1 (dual-gated)
        # z_hat = (1-rho)*z_direct + rho*z_gated
        sample = lq_latent_hat[:, :256]
        x0_pred = self._batched_ddpm_step(model_pred, t_star_long, sample)
        delta_unet = x0_pred - sample
        M_unet = self.gate_unet(scales_all, target_hw=sample.shape[-2:])
        M_aux = self.gate_aux(scales_all, target_hw=sample.shape[-2:])
        z_direct = x0_pred + res1
        z_gated = sample + M_unet * delta_unet + M_aux * res1
        x_denoised = (1.0 - rho) * z_direct + rho * z_gated
    ...
    def decompress(self, strings, shape, pos_prompt, lmbda=None):
        ...
        # A3: Dual-gated fusion at decompress (rho=1, M≈1 at init)
        sample = lq_latent_hat[:, :256]
        x0_pred = self._batched_ddpm_step(model_pred, t_star_long, sample)
        delta_unet = x0_pred - sample
        M_unet = self.gate_unet(scales_all, target_hw=sample.shape[-2:])
        M_aux = self.gate_aux(scales_all, target_hw=sample.shape[-2:])
        x_denoised = sample + M_unet * delta_unet + M_aux * res
        output_image = (
            self.vae.decode(x_denoised / self.vae.config.scaling_factor).sample
        ).clamp(-1, 1)
        return output_image
    ...
    def save_model(self, outf):
        ...
        # A3: Save dual gate modules
        sd["state_dict_gate_unet"] = {k: v for k, v in self.gate_unet.state_dict().items()}
        sd["state_dict_gate_aux"] = {k: v for k, v in self.gate_aux.state_dict().items()}
        torch.save(sd, outf)
