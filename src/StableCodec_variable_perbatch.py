"""
StableCodec_variable.py — StableCodec with variable-rate λ-FiLM conditioning.

Changes from original StableCodec:
  • __init__: no fixed lmbda; accepts lambda_min/lambda_max from config;
              adds unet_lora_proj (Injection Point 3).
  • forward:  lmbda argument (Tensor [B]); computes film_embed from codec.film_embed;
              scales unet model_pred via Injection Point 3.
  • compress: accepts lmbda; stores lmbda_val in output dict (2 bytes/image).
  • decompress: reads lmbda_val from compressed dict; passes to codec.
  • set_train: unfreezes lambda modules (film_embed, unet_lora_proj).
  • save_model: saves lambda module weights.
...
from latent_codec_variable import LatentCodec, LAMBDA_MIN, LAMBDA_MAX
import sys
sys.path.append("..")
...
class StableCodec(torch.nn.Module):
    def __init__(self, sd_path=None, config=None, logger=None, stage=None):
        ...
        # Variable-rate codec: no fixed lambda at construction
        self.codec = LatentCodec(lambda_min=lambda_min, lambda_max=lambda_max)
        ...
        # ----------------------------------------------------------------
        from latent_codec_variable import FILM_DIM
    ...
    def forward(self, x, pos_prompt, ori_h, ori_w, lmbda=None):
        ...
        Args:
            ...
            stage:      int  1 or 2 (controls discriminator activation downstream)
        ...
        lq_latent_hat, RateLossOutput, res1 = self.codec(
        ...
        lora_scale  = (1.0 + torch.tanh(delta_s))           # ∈ (0, 2), scalar

        # ---- One-Step Denoiser ----
        model_pred = self.unet(
            lq_latent_hat, self.timesteps,
        ...
        x_denoised = (
            self.sched.step(model_pred, self.timesteps,
                            lq_latent_hat[:, :256], return_dict=True).prev_sample
            + res1
        )
    ...
    def decompress(self, strings, shape, pos_prompt, lmbda=None):
        ...
        lq_latent_hat, res = self.codec.decompress(strings, shape, lmbda)
        ...
        if lmbda is not None:
            ...
            delta_s    = self.unet_lora_proj(film_embed.mean(0, keepdim=True))
            lora_scale = (1.0 + torch.tanh(delta_s))
        else:
            lora_scale = 1.0
            ...
            for row in range(grid_rows):
                for col in range(grid_cols):
                    ...
                    if len(input_list) == 1 or col == grid_cols - 1:
                        pred = self.unet(
                            torch.cat(input_list, 0), self.timesteps,
                            encoder_hidden_states=pos_caption_enc
                        ).sample * lora_scale
                        input_list = []
        ...
        x_denoised = (
            self.sched.step(model_pred, self.timesteps,
                            lq_latent_hat[:, :4], return_dict=True).prev_sample
            + res
        )
        output_image = (
            self.vae.decode(x_denoised / self.vae.config.scaling_factor).sample
