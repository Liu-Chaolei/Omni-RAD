...
def load_train_objs(train_dataloader, stage, train_config, val_config, world_size, device, rank, save_path, accelerator):
    # Create structure_logger first so it can be passed into StableCodec for
    # unfreeze_mismatched_layers to write missing/unexpected key info to the log file.
    if accelerator.is_main_process:
        structure_logger = logger_setup(log_file_name='structure.log', log_file_folder_name=os.path.join(save_path, 'logs'))
    else:
...
# CUDA_VISIBLE_DEVICES=3 accelerate launch --num_processes 1 src/trainv1.py --stage 2 --MASTER_PORT 12356 --experiment_name channel4_ft2
