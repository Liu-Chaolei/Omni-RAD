"""
StableCodec_variable2_step_A1.py — A1 variant: adds UncertaintyGateModule for
spatially-varying repair allocation on top of the dynamic timestep T* baseline.

A1 changes vs. A0 (StableCodec_variable2_step.py):
  • Import: uses latent_codec_variable2_step_A1 (5-tuple forward, 4-tuple decompress).
  • UncertaintyGateModule: spatial gate M ∈ [0,1]^{B×1×H/8×W/8} derived from
    entropy model scales_all via normalize → bilinear ×4 upsample → sigmoid(α·s+β).
  ...
  • forward fusion (with rho transition):
      ...
      z_gated  = x0_pred + M * res1  (gated Aux residual)
...
from latent_codec_variable2_step_A1 import LatentCodec, LAMBDA_MIN, LAMBDA_MAX
import sys
sys.path.append("..")
...
class UncertaintyGateModule(nn.Module):
    """A1: Spatial uncertainty gate M ∈ [0,1]^{B×1×H/8×W/8}.

    Controls the injection strength of AuxDecoder residual (res1).
    Init: α=0, β=4 → sigmoid(4)≈0.98 → M≈1 at start (identity w.r.t. Stage I).
    Fusion: z_hat = x0_pred + M * res1
    When M=1: equivalent to Stage I's x0_pred + res1.
    """
...
class StableCodec(torch.nn.Module):
    def __init__(self, sd_path=None, config=None, logger=None, stage=None):
        ...
        # ----------------------------------------------------------------
        from latent_codec_variable2_step_A1 import FILM_DIM
        ...
        # A1: Spatial uncertainty gate module
        self.gate_module = UncertaintyGateModule()
        ...
        if config.get('codec_path') is not None:
            ...
            # A1: gate_module (may not exist when warm-starting from A0 checkpoint)
            if "state_dict_gate" in sd:
                self.gate_module.load_state_dict(sd["state_dict_gate"])
                print("  -> Loaded gate_module weights.")
            else:
                print("  -> No gate_module weights in checkpoint; using init (alpha=0, beta=0).")
    ...
    def forward(self, x, pos_prompt, ori_h, ori_w, lmbda=None, rho=1.0):
        ...
        # A1: Gated fusion — M controls Aux residual injection strength
        # z_direct = x0_pred + res1 (Stage I formula)
    ...
    def decompress(self, strings, shape, pos_prompt, lmbda=None):
        ...
        # A1: Gated fusion at decompress (rho=1 at inference, M≈1 at init)
        sample = lq_latent_hat[:, :256]
        x0_pred = self._batched_ddpm_step(model_pred, t_star_long, sample)
        M = self.gate_module(scales_all, target_hw=sample.shape[-2:])
