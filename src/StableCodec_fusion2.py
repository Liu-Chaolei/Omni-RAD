...
from cft_fuse import CFTFuseBlock
...
class StableCodec(torch.nn.Module):
    ...
    def __init__(self, sd_path=None, lmbda=0.5, config=None, logger=None, stage=None):
        ...
        self.cft_w = getattr(config, "cft_w", 0.5)  # 0=diff支路, 1=res支路
        self._res_feat = None
        ...
        if config['codec_path'] is not None:
            ...
            if "state_dict_cft" in sd:
                self.cft_blocks.load_state_dict(sd["state_dict_cft"])
        ...
        def hook_fn(module, input, output):
            if self._res_feat is not None:
                output = self.cft_blocks[f"up_{block_idx}"](
                    self._res_feat, output, self.cft_w
                )
            return output
        return hook_fn

    def _decode_with_cft(self, z: torch.Tensor, res_feat: torch.Tensor, w: float = None):
        if w is not None:
            self.cft_w = w
    ...
    def set_train(self, stage: int = 1):
        """
        Configure trainable parameters and cft_w for the requested stage.

        Stage 1   cft_w=0.5 : full end-to-end training.
        Stage 2   cft_w=0   : x_diff path only  (codec.g_s / UNet / VAE-dec front half).
        Stage 3   cft_w=1   : res1  path only   (codec.aux / CFT blocks).
        Stage 4   cft_w=0.5 : full end-to-end fine-tuning (same as stage 1).
        """
        dispatch = {
    ...
    def _set_train_stage1(self):
        ...
        Trainable components:
          - codec  (all params, including quantiles for aux_loss)
          - unet   (LoRA layers + conv_in)
          - vae    (LoRA layers)
          - cft_blocks (all params)
        """
        self.cft_w = 0.5
        ...
        # codec – unfreeze entirely (includes entropy bottleneck quantiles)
        self.codec.requires_grad_(True)

        # unet – LoRA adapters + custom conv_in only
        for n, p in self.unet.named_parameters():
            if "lora" in n:
                p.requires_grad = True
        self.unet.conv_in.requires_grad_(True)

        # vae – LoRA adapters only (encoder-side LoRA added in __init__)
    ...
    def _set_train_stage2(self):
        """
        Stage 2: cft_w=0, x_diff path only.

        Trainable components:
          - codec.g_s      (latent synthesis / decoder transform)
          - unet           (LoRA layers + conv_in)
          - vae.decoder    front half:
              conv_in + mid_block + up_blocks[0 .. ceil(N/2)-1]

        Everything else (codec entropy models, vae encoder, cft_blocks) is frozen.
        With cft_w=0 the CFT hooks contribute nothing; keeping cft_blocks in eval
        avoids unnecessary BN/Dropout side-effects.
        """
        self.cft_w = 0.0
        self._freeze_all_trainable()

        # train mode only for the active sub-graphs
        self.unet.train()
        self.vae.train()       # decoder layers need train() for BN/Dropout
        self.codec.eval()      # entropy model stays eval
        self.codec.g_s.train()
        self.cft_blocks.eval()

        # codec.g_s – full unfreeze
        ...
        # vae decoder – front half
        # "front half" = conv_in + mid_block + first ceil(N/2) up_blocks
    ...
    def _set_train_stage3(self):
        """
        Stage 3: cft_w=1, res1 path only.

        Trainable components:
          - codec.aux   (auxiliary decoder that produces res1)
          - cft_blocks  (CFT fusion modules)

        UNet and VAE are fully frozen; they only provide the base x_diff latent
        which the CFT modulates (gradient does NOT need to flow back through UNet).
        """
        self.cft_w = 1.0
        self._freeze_all_trainable()
    ...
    def _set_train_stage4(self):
        """
        Stage 4: cft_w=0.5, full end-to-end fine-tuning.
    ...
    def forward(self, x, pos_prompt, ori_h, ori_w):
        ...
        # Ensure scheduler / timesteps live on the same device as input
        device = x.device
