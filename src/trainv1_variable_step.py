"""
stablecodec-trainv1_variable_step.py — Training script for variable-rate StableCodec
with SNR-based dynamic timestep T*.

Key changes from trainv1_variable2.py:
  1. Uses StableCodec_variable_step (which uses DynamicTimestepModule in LatentCodec).
  2. Per-batch λ sampling (log-uniform) before every forward call (unchanged).
  3. Per-image λ-weighted loss: L = mean(λ_i * D_i) + mean(λ_i * bpp_i) (unchanged).
  4. RateLossOutput.rate_loss already uses variable λ (unchanged).
  5. forward() now returns 3 values: (output_image, RateLossOutput, T_star).
  6. Logs Train/T_star_mean and Train/T_star_std to monitor dynamic timestep distribution.
  7. save_model / checkpoint include unet_lora_proj + DynamicTimestepModule weights
     (DynamicTimestepModule is a submodule of LatentCodec, included in state_dict_codec).
"""
...
from StableCodec_variable_step import StableCodec
    ...
    while global_step < train_config['max_train_steps']:
        ...
        for i, d in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                ...
                # --------------------------------------------------------
                # Variable-rate loss: per-image λ weighting
                #
                # Correct formulation:  L = mean_i( λ_i * D_i + bpp_i )
                # per_image_distortion: [B], per-image MSE+LPIPS
                # TargetRateModule.rate_loss already = mean(λ_i * bpp_i)
                # We compute mean(λ_i * D_i) here for the distortion term.
                # --------------------------------------------------------
                per_image_dist = out_criterion["per_image_distortion"]  # [B]
                weighted_distortion = (lmbda * per_image_dist).mean()
                ...
                # Per-image λ-weighted: L = mean(λ_i * D_i) + mean(λ_i * bpp_i)
                generator_loss  = weighted_distortion + out_criterion['bpp_loss']
...
# CUDA_VISIBLE_DEVICES=6 accelerate launch --num_processes 1 trainv1_variable_step.py --stage 2 --MASTER_PORT 12356 --experiment_name ft2_step999_new
