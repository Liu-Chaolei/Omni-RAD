"""
stablecodec-trainv1_variable3_debug_step5.py — Training script for variable-rate StableCodec
with D/λ + bpp loss, debug diagnostics, SNR-based dynamic timestep T*, and curriculum learning.

Key changes from trainv1_variable3_debug_step.py:
  1. Uses StableCodec_variable2_step5 (supports t_override for curriculum learning).
  2. Curriculum learning (5C): starts training at T=999 (UNet's comfort zone), then
     gradually widens the UNet timestep range to [t_min, 999] over warmup_steps.
     current_t_min = int(999 - (999 - t_min) * min(global_step / warmup_steps, 1.0))
     At each step, t_curriculum ~ Uniform[current_t_min, 1000) is sampled per image
     and passed as t_override. The codec's natural T_star is still computed and logged.
  3. Logs Train/T_used_mean, Train/curriculum_t_min alongside T_star metrics.
  4. diag_buffer collects both T* (codec natural) and T_used (curriculum override).
"""
...
from StableCodec_variable2_step5 import StableCodec
...
def main(stage, base_config, train_config, val_config, MASTER_PORT, experiment_name):
    ...
    # -------------------- Curriculum learning (5C) --------------------
    curriculum_warmup_steps = int(train_config['model'].get('curriculum_warmup_steps', 10000))
    curriculum_t_min = int(train_config['model'].get('curriculum_t_min', 800))
    curriculum_t_max = 999
    ...
    def _log_diag_and_clear(logger, step):
        ...
        tused = np.asarray(diag_buffer["t_used"], dtype=np.float64)
        if lam.size == 0:
            return
    ...
    while global_step < train_config['max_train_steps']:
        ...
        for i, d in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                ...
                # ---- Curriculum learning: compute current timestep range ----
                progress = min(global_step / curriculum_warmup_steps, 1.0)
                current_t_min = int(curriculum_t_max - (curriculum_t_max - curriculum_t_min) * progress)
                t_curriculum = torch.randint(current_t_min, curriculum_t_max + 1, (B,),
                                            device=device).float()
                ...
                with torch.no_grad():
                    ...
                    diag_buffer["t_used"].extend(t_curriculum.detach().cpu().numpy().tolist())
                ...
                current_metrics = {
                    ...
                    'Train/T_used_mean':      t_curriculum.mean().item(),
                    'Train/curriculum_t_min':  float(current_t_min),
                }
