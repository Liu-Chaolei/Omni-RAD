...
class ContentAdaptiveFusion(nn.Module):
    ...
    def forward(
        ...
        if stage == 2:
            # stage 2：res1 权重为 0，意味着 1 - alpha_map = 0，即 alpha_map = 1
            alpha_map = torch.ones((B, 1, h, w), dtype=feat.dtype, device=feat.device)
        elif stage == 3:
            # stage 3：x_diff 权重为 0，即 alpha_map = 0
            alpha_map = torch.zeros((B, 1, h, w), dtype=feat.dtype, device=feat.device)
        else:
            # 其他 stage (1, 4) 由神经网络动态学习融合权重
            alpha_map = self.net(feat)   
...
class StableCodec(torch.nn.Module):
    ...
    def set_train(self, stage=1):
        ...
        if stage == 1 or stage == 4:
            ...
            # Freeze all net parameters except net.unet and net.codec.g_s
            for n, _p in self.unet.named_parameters():
                if "lora" in n:
                    _p.requires_grad = True
            self.unet.conv_in.requires_grad_(True)
            for p in self.codec.g_s.parameters():
                p.requires_grad = True
            for n, p in self.codec.named_parameters():
                if n.endswith(".quantiles"):
                    p.requires_grad = True
        elif stage == 3:
            # Freeze all net parameters except net.codec.aux
            for p in self.codec.aux.parameters():
                p.requires_grad = True
            for n, p in self.codec.named_parameters():
                if n.endswith(".quantiles"):
                    p.requires_grad = True
    ...
    def forward(self, x, pos_prompt, ori_h, ori_w):
        ...
        # One-Step Denoiser
        if self.stage == 3:
            x_diff = torch.zeros_like(lq_latent_hat[:, :self.latent_channels])
        else:
        ...
        x_denoised, _ = self.fusion(x_diff, res1, lq_latent_hat, self.lambda_rate, self.stage)
    ...
    def decompress(self, strings, shape, pos_prompt):
        ...
        x_denoised, _ = self.fusion(x_diff, res, lq_latent_hat, self.lambda_rate, self.stage)
