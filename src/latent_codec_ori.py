...
############################################################################32
class AnalysisTransform_256(nn.Module):
    def __init__(self):
        super().__init__()
...
class AuxDecoder_256(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.block = nn.Sequential(
            BasicBlock(320),
...
class LatentCodec(CompressionModel):
    def __init__(self, lambda_rate, latent_channels=256):
        super().__init__()
        ...
        if latent_channels == 256:
            self.g_a = AnalysisTransform_256()
            self.aux = AuxDecoder_256()
        elif latent_channels == 4:
            self.g_a = AnalysisTransform_4()
            self.aux = AuxDecoder_4()
        else:
            raise ValueError("Unsupported latent channels")
