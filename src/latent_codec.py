...
############################################################################
class AnalysisTransform_ori(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(self, latent, latent2): # latent: vae；latent2: Eaux
        x = torch.cat((self.pre1(latent), self.pre2(latent2)), dim=1)
        x = self.analysis_transform(x)
        return x

class AuxDecoder_ori(nn.Module):
...
############################################################################11
class AnalysisTransform11(nn.Module):
    def __init__(self):
        super().__init__()
...
############################################################################12
class AnalysisTransform12(nn.Module):
    def __init__(self):
        super().__init__()
        self.pre1 = Downsample(256, 256)
        self.pre2 = nn.Conv2d(320, 64, kernel_size=3, padding=1)
        self.analysis_transform = nn.Sequential(
            BasicBlock(320),
            Downsample(320, 320),
...
class AuxDecoder12(nn.Module):
    ...
    def forward(self, x):
        ...
        return x
############################################################################31
class AnalysisTransform31(nn.Module):
    def __init__(self):
        super().__init__()
        self.pre1 = Downsample(128, 128)
        self.pre2 = nn.Conv2d(320, 64, kernel_size=3, padding=1)
        self.analysis_transform = nn.Sequential(
            BasicBlock(192),
            Downsample(192, 256),
...
class LatentCodec(CompressionModel):
    ...
    def process_group_with_mask(self, gaussian_conditional, latent, scales, means, mask, training):
        latent_squeeze = self.sequeeze_with_mask(latent, mask)
        scales_squeeze = self.sequeeze_with_mask(scales, mask)
        means_squeeze = self.sequeeze_with_mask(means, mask)

        latent_squeeze_hat, likelihood_squeeze = gaussian_conditional(
            latent_squeeze, scales_squeeze, means_squeeze, training=training
        )

        latent_hat = self.unsequeeze_with_mask(latent_squeeze_hat, mask)
        likelihood = self.unsequeeze_with_mask(likelihood_squeeze, mask)
        return latent_hat, likelihood

    def forward(self, latent, latent2, training):
        ...
        #     z_tmp = z - z_offset
        #     z_hat = self.quantize_ste(z_tmp) + z_offset
        # z_hat = self.quantize_ste(z)
        # z_noise = self.quantize_noise(z)
        # z_likelihoods = self.get_likelihoods(z_noise)
        ...
        y_hat_0, y_likelihood_0 = self.process_group_with_mask(self.gaussian_conditional, y, scales_0_supp, means_0_supp, mask_0, training)
        ...
        y_hat_1, y_likelihood_1 = self.process_group_with_mask(self.gaussian_conditional, y, scales_1_supp, means_1_supp, mask_1, training)
        ...
        y_hat_2, y_likelihood_2 = self.process_group_with_mask(self.gaussian_conditional, y, scales_2_supp, means_2_supp, mask_2, training)
        ...
        y_hat_3, y_likelihood_3 = self.process_group_with_mask(self.gaussian_conditional, y, scales_3_supp, means_3_supp, mask_3, training)
        lrp = self.LRP[3](torch.cat([y_hat_3, base], dim=1)) * mask_3
        lrp = 0.5 * torch.tanh(lrp)
        y_hat_3 = y_hat_3 + lrp

        y_hat_total = y_hat_0 + y_hat_1 + y_hat_2 + y_hat_3
        y_likelihoods = y_likelihood_0 + y_likelihood_1 + y_likelihood_2 + y_likelihood_3

        x_hat = self.g_s(y_hat_total)
        res = self.aux(y_hat_total)
