...
Key changes from original:
  ...
  3. Loss rescaling: distortion term is reweighted by  mean(lmbda)/lambda_ref  so
     that the optimiser sees a consistently scaled gradient across all λ values.
...
def log_validation(step, val_dataloader, model, criterion, iqa_psnr,
    ...
    lambda_ref = float(train_config.get('lambda_ref', train_config.get('lmbda', 1.0)))
...
def main(stage, base_config, train_config, val_config, MASTER_PORT, experiment_name):
    ...
    lambda_ref = float(train_config.get('lambda_ref', train_config.get('lmbda', 1.0)))
    lambda_min = float(train_config['model'].get('lambda_min', 0.5))
    lambda_max = float(train_config['model'].get('lambda_max', 32.0))
    ...
    while global_step < train_config['max_train_steps']:
        ...
        for i, d in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                ...
                # --------------------------------------------------------
                # Variable-rate loss rescaling
                #
                # criterion uses a fixed lambda_ref; the rate term from
                # TargetRateModule already uses per-image lmbda.  We rescale
                # the distortion component so the total loss is equivalent to:
                #     L = mean(lmbda) · D + R(lmbda)
                # --------------------------------------------------------
                # mean_lmbda   = lmbda.mean().item()
                # lambda_scale = mean_lmbda / lambda_ref      # rescale ratio
                ...
                # Rescale distortion part, then add variable-rate bpp loss
                # distortion_loss = out_criterion["compression_loss"] * lambda_scale
                # generator_loss  = distortion_loss + out_criterion['bpp_loss']
...
# CUDA_VISIBLE_DEVICES=4 accelerate launch --num_processes 1 src/trainv1_variable_perbatch.py --stage 1 --MASTER_PORT 12355 --experiment_name variable_perbatch_base
