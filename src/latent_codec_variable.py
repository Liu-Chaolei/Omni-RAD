...
class TargetRateModule(nn.Module):
    ...
    lmbda is passed at forward time (not stored at init), supporting:
      • Log-uniform λ sampling per batch during training.
      • Two-stage IBP: Stage I uses λ_base, Stage II uses λ_target range.

    The rate term  λ · bpp  uses per-image λ values, consistent with I2C's
    tensor-Λ training strategy (I2C Eq. 6/7), giving continuous coverage of
    the full rate-distortion curve within a single training run.
    """
    ...
    def forward(
        ...
        lmbda_t = lmbda.to(total_bpp.device) if isinstance(lmbda, torch.Tensor) \
                  else total_bpp.new_tensor(lmbda)
        if lmbda_t.dim() == 0:
            lmbda_t = lmbda_t.expand_as(total_bpp)

        return RateLossOutput(
            rate_loss            = (lmbda_t * total_bpp).mean(),
...
class LatentCodec(CompressionModel):
    ...
    Compared to the original fixed-λ version:
      • film_embed (LambdaFiLMEmbed)  — new: shared λ→f(λ) embedding
      • g_a (AnalysisTransform)       — modified: FiLM at 3 layers
      • g_s (SynthesisTransform)      — modified: FiLM at 3 layers
      • aux (AuxDecoder)              — modified: FiLM at 3 layers
      • rate (TargetRateModule)       — modified: lmbda passed at forward time

    Interface change:
        forward   (latent, latent2, ori_h, ori_w, lmbda: Tensor)
        compress  (latent, latent2, lmbda)  → returns "lmbda_val" in dict
        decompress(strings, shape, lmbda)   → reads from strings["lmbda_val"]
    """
    ...
    def compress(self, latent, latent2, lmbda=None):
        """
        Returns a dict with keys: strings, shape, lmbda_val.
