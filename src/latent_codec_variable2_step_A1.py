"""
latent_codec_variable2_step_A1.py  —  A1 variant of latent codec.

A1 changes vs. A0 (latent_codec_variable2_step.py):
...
UncertaintyGateModule in StableCodec_variable2_step_A1.py to drive a
spatially-varying gate M ∈ [0,1]^{B×1×H/8×W/8} controlling the strength of
diffusion-based denoising vs. trusting the codec-decoded latent.
...
class LatentCodec(CompressionModel):
    ...
    def forward(self, latent, latent2, ori_h, ori_w, lmbda):
        ...
        # A1: expose scales_all so the model-level UncertaintyGateModule can
        # consume it for spatial repair allocation.
    ...
    def decompress(self, strings, shape, lmbda=None):
        ...
        # A1: scales_all must be returned for the model-level gate, so compute
