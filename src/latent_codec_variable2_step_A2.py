"""
latent_codec_variable2_step_A2.py  —  A2 variant of latent codec.

A2 changes vs. A0 (latent_codec_variable2_step.py):
...
UncertaintyGateModule (channel_spatial mode) in StableCodec_variable2_step_A2.py
to drive a channel-spatial gate M ∈ [0,1]^{B×256×H/8×W/8}.
...
class LatentCodec(CompressionModel):
    ...
    def decompress(self, strings, shape, lmbda=None):
        ...
        # A2: scales_all must be returned for the model-level gate, so compute
