"""
StableCodec_variable2_step_A2.py — A2 variant: adds UncertaintyGateModule with
channel-spatial mode for per-channel spatially-varying repair allocation.

A2 changes vs. A0 (StableCodec_variable2_step.py):
  • Import: uses latent_codec_variable2_step_A2 (5-tuple forward, 4-tuple decompress).
  • UncertaintyGateModule (channel_spatial mode): 1×1 Conv(320→256) → bilinear ×4
    upsample → 3×3 Conv(256→256) → normalize → sigmoid(α·feat+β).
    Output M ∈ [0,1]^{B×256×H/8×W/8}. Init α=0, β=4 → M≈0.98 (identity).
  • forward fusion (with rho transition):
      z_direct = x0_pred + res1  (Stage I formula)
      z_gated  = x0_pred + M * res1  (gated Aux residual, per-channel)
      z_hat    = (1-ρ)*z_direct + ρ*z_gated
    When rho=0: equivalent to Stage I. When rho=1, M=1: also equivalent.
  • decompress fusion: z_hat = x0_pred + M * res1 (rho=1 at inference).
  • save_model: additionally saves state_dict_gate.
  • Checkpoint loading: gracefully handles missing state_dict_gate (warm-start from A0).
  • set_train: gate_module parameters are trainable.
...
from latent_codec_variable2_step_A2 import LatentCodec, LAMBDA_MIN, LAMBDA_MAX
import sys
sys.path.append("..")
...
class UncertaintyGateModule(nn.Module):
    """A2: Channel-spatial uncertainty gate M ∈ [0,1]^{B×256×H/8×W/8}.

    Uses a lightweight conv pipeline to project entropy model scales (320ch)
    to per-channel gate maps (256ch) at VAE latent resolution.
    Pipeline: 1×1 Conv(320→256) → bilinear ×4 → 3×3 Conv(256→256) → normalize → sigmoid.
    Init: α=0, β=4 → sigmoid(4)≈0.98 → M≈1 at start (identity w.r.t. Stage I).
    Fusion: z_hat = x0_pred + M * res1
    """

    def __init__(self, in_channels: int = 320, out_channels: int = 256):
        super().__init__()
    ...
    def forward(self, scales_all: torch.Tensor, target_hw: tuple) -> torch.Tensor:
        feat = self.proj(scales_all)
        feat = F.interpolate(feat, size=target_hw, mode='bilinear', align_corners=False)
        feat = self.refine(feat)
        feat = (feat - feat.mean(dim=[2, 3], keepdim=True)) / (feat.std(dim=[2, 3], keepdim=True) + 1e-6)
        return torch.sigmoid(self.alpha * feat + self.beta)
...
class StableCodec(torch.nn.Module):
    def __init__(self, sd_path=None, config=None, logger=None, stage=None):
        ...
        # ----------------------------------------------------------------
        from latent_codec_variable2_step_A2 import FILM_DIM
        ...
        # A2: Channel-spatial uncertainty gate module
        self.gate_module = UncertaintyGateModule(in_channels=320, out_channels=256)
        ...
        if config.get('codec_path') is not None:
            ...
            # A2: gate_module (may not exist when warm-starting from A0 checkpoint)
            if "state_dict_gate" in sd:
                self.gate_module.load_state_dict(sd["state_dict_gate"])
                print("  -> Loaded gate_module weights.")
            else:
                print("  -> No gate_module weights in checkpoint; using default init.")
    ...
    def forward(self, x, pos_prompt, ori_h, ori_w, lmbda=None, rho=1.0):
        ...
        Args:
            ...
            rho:        float in [0,1]. Transition from direct fusion (0) to
                        gated fusion (1). Training loop schedules this.
        ...
        # A2: Gated fusion — M controls Aux residual injection strength (per-channel)
        # z_direct = x0_pred + res1 (Stage I formula)
        # z_gated = x0_pred + M * res1 (gated formula)
        # z_hat = (1-rho)*z_direct + rho*z_gated
        sample = lq_latent_hat[:, :256]
        x0_pred = self._batched_ddpm_step(model_pred, t_star_long, sample)
        M = self.gate_module(scales_all, target_hw=sample.shape[-2:])
        z_direct = x0_pred + res1
        z_gated = x0_pred + M * res1
    ...
    def decompress(self, strings, shape, pos_prompt, lmbda=None):
        ...
        # A2: Gated fusion at decompress (rho=1 at inference, M≈1 at init)
        sample = lq_latent_hat[:, :256]
        x0_pred = self._batched_ddpm_step(model_pred, t_star_long, sample)
        M = self.gate_module(scales_all, target_hw=sample.shape[-2:])
        x_denoised = x0_pred + M * res
        output_image = (
            self.vae.decode(x_denoised / self.vae.config.scaling_factor).sample
        ).clamp(-1, 1)
        return output_image
    ...
    def save_model(self, outf):
        ...
        # A2: Save gate module
        sd["state_dict_gate"] = {k: v for k, v in self.gate_module.state_dict().items()}
        torch.save(sd, outf)
