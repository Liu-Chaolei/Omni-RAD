"""
stablecodec-trainv1_variable3_debug_step6.py — Variable-rate StableCodec training with
non-uniform λ sampling and rate constraint, on top of D/λ + bpp loss + dynamic T*.

Changes from trainv1_variable3_debug_step.py:
  1. sample_lambda_truncnorm: truncated normal in log-space, concentrating density
     around a configurable centre while keeping wide tail support for regularisation.
  2. Rate constraint penalty: α·mean(relu(bpp_lo - bpp)² + relu(bpp - bpp_hi)²)
     pushes per-image bpp into a target corridor, with linear warmup.
  3. New YAML config fields under `model`:
       lambda_sample_mode   ("uniform" | "truncnorm",  default "uniform")
       lambda_sample_mu     (log-space centre,          default log(4))
       lambda_sample_sigma  (log-space spread,          default 0.8)
       rate_constraint_alpha   (penalty weight,         default 0.0)
       bpp_lo / bpp_hi         (corridor bounds,        default 0.01 / 0.06)
       rate_constraint_warmup  (linear ramp-up steps,   default 1000)
  4. Enhanced diagnostics: rate_penalty, bpp_in_range_ratio.
"""
...
def sample_lambda_truncnorm(
    ...
    """Sample B λ values from a truncated normal in log-space.

    Concentrates sampling density around exp(mu) while keeping support over
    the full [lambda_min, lambda_max] range for regularisation.
    """
    log_min = math.log(lambda_min)
    log_max = math.log(lambda_max)
    a = (log_min - mu) / sigma
    ...
    samples = _truncnorm.rvs(a, b, loc=mu, scale=sigma, size=B)
    return torch.tensor(samples, dtype=torch.float32, device=device).exp()
...
def log_validation(step, val_dataloader, model, criterion, iqa_psnr,
    ...
    with torch.no_grad():
        ...
        for d in val_dataloader:
            ...
            x_hat, RateLossOutput, T_star = model(d, pos_tag_prompt, H, W, lmbda_val)
...
def main(stage, base_config, train_config, val_config, MASTER_PORT, experiment_name):
    ...
    # --- Non-uniform sampling & rate constraint config ---
    lambda_sample_mode = str(train_config['model'].get('lambda_sample_mode', 'uniform'))
    lambda_sample_mu   = float(train_config['model'].get('lambda_sample_mu', math.log(4.0)))
    lambda_sample_sigma = float(train_config['model'].get('lambda_sample_sigma', 0.8))
    rate_constraint_alpha = float(train_config['model'].get('rate_constraint_alpha', 0.0))
    bpp_lo = float(train_config['model'].get('bpp_lo', 0.01))
    bpp_hi = float(train_config['model'].get('bpp_hi', 0.06))
    rate_constraint_warmup = int(train_config['model'].get('rate_constraint_warmup', 1000))
    ...
    metric_names = ['Train/loss','Train/mse_loss','Train/lpips','Train/bpp',
                    ...
                    'Train/T_star_mean','Train/T_star_std','Train/rate_penalty']

    # -------------------- λ-BPP-T* diagnostics --------------------
    diag_every    = int(train_config.get('diag_every', 500))
    diag_num_bins = int(train_config.get('diag_num_bins', 6))
    diag_buffer   = {"lambda": [], "bpp": [], "dist": [], "lambda_dist": [], "t_star": [], "rate_penalty": []}
    ...
    def _log_diag_and_clear(logger, step):
        ...
        tstar = np.asarray(diag_buffer["t_star"], dtype=np.float64)
        rpen  = np.asarray(diag_buffer["rate_penalty"], dtype=np.float64)
        if lam.size == 0:
            return
        loglam = np.log(np.clip(lam, 1e-12, None))
        in_range = np.mean((bpp >= bpp_lo) & (bpp <= bpp_hi))
        msg = (f"[DIAG step={step}] n={lam.size} "
               ...
               f"corr(logλ,T*)={_safe_corr(loglam, tstar):.4f} "
               f"mean_bpp={bpp.mean():.6f} "
               f"range_bpp=({bpp.min():.6f},{bpp.max():.6f}) "
               f"mean_T*={tstar.mean():.1f} "
               f"bpp_in_range={in_range:.3f} "
               f"mean_rate_pen={rpen.mean():.6f}")
        print(msg)
    ...
    while global_step < train_config['max_train_steps']:
        ...
        for i, d in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                ...
                # Sample per-image λ
                # --------------------------------------------------------
                if lambda_sample_mode == 'truncnorm':
                    lmbda = sample_lambda_truncnorm(
                        B, lambda_min, lambda_sample_max,
                        lambda_sample_mu, lambda_sample_sigma, device)
                else:
                ...
                with torch.no_grad():
                    ...
                    diag_buffer["t_star"].extend(T_star.detach().cpu().numpy().tolist())
                ...
                # --------------------------------------------------------
                # Rate constraint penalty:
                #   α · mean(relu(bpp_lo - bpp)² + relu(bpp - bpp_hi)²)
                # with linear warmup over rate_constraint_warmup steps.
                # --------------------------------------------------------
                per_image_bpp = RateLossOutput.per_image_bpp  # [B]
                rate_pen_per_img = (
                    F.relu(bpp_lo - per_image_bpp).square()
                    + F.relu(per_image_bpp - bpp_hi).square()
                )
                warmup_factor = min(1.0, global_step / max(rate_constraint_warmup, 1))
                rate_penalty = rate_constraint_alpha * warmup_factor * rate_pen_per_img.mean()

                with torch.no_grad():
                    diag_buffer["rate_penalty"].extend(rate_pen_per_img.detach().cpu().numpy().tolist())

                # L = mean(D_i / λ_i) + mean(bpp_i) + rate_penalty
                generator_loss  = inv_lambda_distortion + out_criterion['bpp_loss'] + rate_penalty
                ...
                current_metrics = {
                    ...
                    'Train/rate_penalty': rate_penalty.item(),
                }
