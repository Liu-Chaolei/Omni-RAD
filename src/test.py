"""Omni-RAD forward testing: estimated bpp, PSNR, VGG-LPIPS, and timestep statistics."""
from cli import parse_args_testing
...
def main(base_config, test_config, stage):
    ...
    images = images[:test_config.get('max_images')]
    if not images:
    ...
    for lmbda_val in lambda_list:
        ...
        for img_path in images:
            ...
            img_padded = F.pad(img, pad=(0, pad_w, 0, pad_h), mode='reflect' if pad_h < ori_h and pad_w < ori_w else 'replicate')
            ...
            with torch.no_grad():
                try:
                    ...
                    lmbda_tensor = torch.tensor([lmbda_val],
                                                dtype=torch.float32,
                                                device=device)
                    x_hat, RateLossOutput, T_star = net(img_padded, pos_tag_prompt,
                                                ori_h, ori_w, lmbda=lmbda_tensor)

                    bpp = RateLossOutput.quantized_total_bpp.item()
                    cur_tstar = T_star.mean().item()
                    ...
                    tstar_meter.update(cur_tstar)
                    bpp_list.append(bpp)

                    print(f'  BPP: {bpp:.4f} | PSNR: {cur_psnr:.2f} | '
                          f'LPIPS: {cur_lpips:.4f} | T*: {cur_tstar:.1f}')
        ...
        avg_tstar = tstar_meter.avg
        all_results[lmbda_val] = {
            'bpp': avg_bpp, 'psnr': avg_psnr, 'lpips': avg_lpips, 't_star': avg_tstar,
        }

        print(f'\n  [{tag}] Avg BPP: {avg_bpp:.4f} | '
              f'PSNR: {avg_psnr:.2f} | LPIPS: {avg_lpips:.4f} | T*: {avg_tstar:.1f}')

    import json
    with open(os.path.join(test_config['rec_path'], 'metrics.json'), 'w') as f:
        json.dump({'rate_type': 'estimated_bpp', 'results': all_results}, f, indent=2)

    # Summary table
    print(f'\n\n{"="*78}')
    print(f'  Variable-Rate RD Summary (with Dynamic Timestep T*)')
    print(f'{"="*78}')
    print(f'  {"Lambda":>10s}  {"BPP":>8s}  {"PSNR":>8s}  {"LPIPS":>8s}  {"T*":>6s}')
    print(f'  {"-"*10}  {"-"*8}  {"-"*8}  {"-"*8}  {"-"*6}')
    for lmbda_val in lambda_list:
        r = all_results.get(lmbda_val)
        if r:
            print(f'  {lmbda_val:10.3f}  {r["bpp"]:8.4f}  '
                  f'{r["psnr"]:8.2f}  {r["lpips"]:8.4f}  {r["t_star"]:6.1f}')
    print(f'{"="*78}\n')
