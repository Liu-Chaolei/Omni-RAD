"""
StableCodec_variable_step.py — StableCodec with variable-rate λ-FiLM conditioning
and SNR-based dynamic timestep T*.

Changes from StableCodec_variable.py:
  • Import: uses latent_codec_variable_step (adds DynamicTimestepModule).
  ...
  • compress: codec.compress now stores t_star_val in output dict automatically.
  • decompress: codec.decompress returns (x_hat, res, T_star); uses T_star for UNet.
  • save_model: DynamicTimestepModule is a submodule of LatentCodec, included in
                state_dict_codec — no extra saving needed.
...
from latent_codec_variable_step import LatentCodec, LAMBDA_MIN, LAMBDA_MAX
import sys
sys.path.append("..")
...
class StableCodec(torch.nn.Module):
    def __init__(self, sd_path=None, config=None, logger=None, stage=None):
        ...
        # ----------------------------------------------------------------
        from latent_codec_variable_step import FILM_DIM
    ...
    def forward(self, x, pos_prompt, ori_h, ori_w, lmbda=None):
        ...
        # T_star is [B] float; UNet and DDPMScheduler both accept batched timesteps
