...
class StableCodec(torch.nn.Module):
    def __init__(self, sd_path=None, lmbda=0.5, config=None, logger=None, stage=None):
        super().__init__()
        self._structure_logger = logger  # used by unfreeze_mismatched_layers below
        self.stage = stage
        ...
        self.codec = LatentCodec(lmbda, self.latent_channels)
    ...
    def forward(self, x, pos_prompt, ori_h, ori_w):
        ...
        x_denoised = self.sched.step(model_pred, self.timesteps, lq_latent_hat[:, :self.latent_channels], return_dict=True).prev_sample + res1
    ...
    def decompress(self, strings, shape, pos_prompt):
        ...
        x_denoised = self.sched.step(model_pred, self.timesteps, lq_latent_hat[:, :self.latent_channels], return_dict=True).prev_sample + res
