"""
stablecodec-trainv1_variable.py — Training script for variable-rate StableCodec.

Key changes from original:
  1. StableCodec no longer takes a fixed 'lmbda'; config needs lambda_min/lambda_max.
  2. Per-batch λ sampling (log-uniform) before every forward call.
  3. Per-image λ-weighted loss: L = mean(λ_i * D_i) + mean(λ_i * bpp_i),
     ensuring each image gets the correct rate-distortion trade-off gradient.
  4. RateLossOutput.rate_loss already uses variable λ (handled inside TargetRateModule).
  5. save_model / checkpoint include unet_lora_proj weights.
"""
...
def log_validation(step, val_dataloader, model, criterion, iqa_psnr,
    ...
    with torch.no_grad():
        ...
        for d in val_dataloader:
            ...
            x_hat, RateLossOutput = model(d, pos_tag_prompt, H, W, lmbda_val)
    ...
    metrics = torch.tensor(
        [loss.sum, bpp.sum, mse_loss.sum, lpips.sum, psnr.sum, loss.count],
        device=device)
...
def main(stage, base_config, train_config, val_config, MASTER_PORT, experiment_name):
    ...
    diag_buffer   = {"lambda": [], "bpp": [], "dist": [], "lambda_dist": []}
    ...
    while global_step < train_config['max_train_steps']:
        ...
        for i, d in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                ...
                # --------------------------------------------------------
                # Sample per-image λ  (log-uniform in [lambda_min, lambda_max])
                # --------------------------------------------------------
                lmbda = sample_lambda(B, lambda_min, lambda_max, device)

                x_hat, RateLossOutput = net(d, pos_tag_prompt,
