...
from StableCodec_fusion4 import StableCodec
...
def load_train_objs(train_dataloader, stage, train_config, val_config, world_size, device, rank, save_path, accelerator):
    ...
    optimizer, aux_optimizer, disc_optimizer = configure_optimizers(net, net_disc, train_config, world_size, stage, rank, save_path, accelerator, structure_logger=structure_logger)
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % train_config['checkpointing_steps'] == 0 and global_step > 0:
                    if accelerator.is_main_process:
                        ...
                        raw_unet = unwrap_model(accelerator, net).unet._orig_mod if hasattr(unwrap_model(accelerator, net).unet, '_orig_mod') else unwrap_model(accelerator, net).unet
                        ...
                        vae_raw_sd  = get_raw_state_dict(unwrap_model(accelerator, net).vae)
                        unet_raw_sd = get_raw_state_dict(unwrap_model(accelerator, net).unet)

                        vae_mismatched_keys  = _extract_mismatched_keys(unwrap_model(accelerator, net).vae_loading_info,  vae_raw_sd)
                        unet_mismatched_keys = _extract_mismatched_keys(unwrap_model(accelerator, net).unet_loading_info, unet_raw_sd)
                        ckpt_dict = {
                                "epoch": epoch,
                                "global_step": global_step,
                                "state_dict_codec": unwrap_model(accelerator, net).codec.state_dict(),
                                "state_dict_aux_codec": unwrap_model(accelerator, net).aux_codec.state_dict(),
...
# CUDA_VISIBLE_DEVICES=5 accelerate launch --num_processes 1 src/trainv1_all4.py --stage 2 --MASTER_PORT 12356 --experiment_name ft2_step999_new
