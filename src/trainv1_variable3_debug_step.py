"""
stablecodec-trainv1_variable3_debug_step.py — Training script for variable-rate StableCodec
with D/λ + bpp loss, debug diagnostics, and SNR-based dynamic timestep T*.

Key changes from trainv1_variable3_debug.py:
  1. Uses StableCodec_variable2_step (which uses DynamicTimestepModule in LatentCodec).
  2. forward() now returns 3 values: (output_image, RateLossOutput, T_star).
  3. Logs Train/T_star_mean and Train/T_star_std to monitor dynamic timestep distribution.
  4. diag_buffer collects T* per image; _log_diag_and_clear reports corr(logλ, T*)
     and per-bin mean_T* for analysing the λ→T* relationship.
  5. DynamicTimestepModule is a submodule of LatentCodec, included in state_dict_codec.
"""
...
def main(stage, base_config, train_config, val_config, MASTER_PORT, experiment_name):
    ...
    metric_names = ['Train/loss','Train/mse_loss','Train/lpips','Train/bpp',
                    ...
                    'Train/T_star_mean','Train/T_star_std','Train/rho']
    ...
    def _log_diag_and_clear(logger, step):
        ...
        if lam.size == 0:
            return
        # Align buffer lengths (T_star may have fewer entries if returned as scalar)
        n = min(lam.size, bpp.size, dist.size, lamdist.size, tstar.size)
        lam, bpp, dist, lamdist, tstar = lam[:n], bpp[:n], dist[:n], lamdist[:n], tstar[:n]
    ...
    while global_step < train_config['max_train_steps']:
        ...
        for i, d in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                ...
                # ---- rho schedule for CFT / gated fusion ----
                if rho_warmup_steps > 0:
                    rho = min(rho_final, max(0.0, (global_step - rho_start_step) / rho_warmup_steps) * rho_final)
                else:
                    rho = rho_final
