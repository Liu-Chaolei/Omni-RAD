...
from my_utils.utils import configure_module_gradients
...
class StableCodec(torch.nn.Module):
    def __init__(self, stage=None, config=None):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(config['sd_path'], subfolder="tokenizer")
        self.text_encoder = CLIPTextModel.from_pretrained(config['sd_path'], subfolder="text_encoder").to('cuda')
        self.sched = make_1step_sched(config['sd_path'])
        ...
        if config['codec_path'] is not None:
            ...
            if config["resume_train"]:
                self.skipped_codec = sd["skipped_codec"]
                self.skipped_vae = sd["skipped_vae"]
                self.skipped_unet = sd["skipped_unet"]
            ...
            configure_module_gradients(self.codec, "Codec", self.skipped_codec, stage)
            configure_module_gradients(self.vae, "VAE", self.skipped_vae, stage)
            configure_module_gradients(self.unet, "UNet", self.skipped_unet, stage)
        total_trainable = sum(p.numel() for p in self.parameters() if p.requires_grad)
        print(f"[Training Setup] Total Trainable Parameters in System: {total_trainable}")
        ...
        if config['freeze_aux_encoder']:
            self.aux_codec.eval()
            self.aux_codec.requires_grad_(False)
        else:
            self.aux_codec.train()
            self.aux_codec.requires_grad_(True)
        if "state_dict_aux_codec" in sd:
            self.aux_codec.load_state_dict(sd["state_dict_aux_codec"])
        del sd
    ...
    def forward(self, c_t, freeze_aux=True, training=True):
        # Encoder
        if freeze_aux:
            latent2 = self.aux_codec((c_t + 1) / 2).detach()
        else:
            latent2 = self.aux_codec((c_t + 1) / 2)
        lq_latent = self.vae.encode(c_t).latent_dist.mode() * self.vae.config.scaling_factor

        # Latent Codec - Entropy Encoding
        output_dict = self.codec(lq_latent, latent2, training)
        lq_latent_hat = output_dict['x_hat']
        res = output_dict['res']

        pos_caption_enc = [self.pos_caption_enc for i in range(c_t.size()[0])]
        ...
        self.timesteps = self.timesteps.to(c_t.device)
        if h * w <= tile_size * tile_size:
            ...
            noise_pred = torch.zeros(lq_latent_hat[:, :256].shape, device=lq_latent_hat.device)
            contributors = torch.zeros(lq_latent_hat[:, :256].shape, device=lq_latent_hat.device)
            for row in range(grid_rows):
                for col in range(grid_cols):
                    if col < grid_cols-1 or row < grid_rows-1:
                        ofs_x = max(row * tile_size-tile_overlap * row, 0)
        ...
        self.timesteps = self.timesteps.to(c_t.device)
        self.sched.timesteps = self.sched.timesteps.to(c_t.device)
        ...
        # change channel
        x_denoised = self.sched.step(model_pred, self.timesteps, lq_latent_hat[:, :256], return_dict=True).prev_sample + res
        ...
        return {
            "x_hat": output_image,
            "likelihoods": {
                "y": output_dict['likelihoods']['y'],
                "z": output_dict['likelihoods']['z']
            }
        }
    ...
    def decompress(self, strings, shape, pos_tag_prompt):

        # Latent Codec - Entropy Decoding
        lq_latent_hat, res = self.codec.decompress(strings, shape)

        pos_caption_enc = [self.pos_caption_enc for i in range(len(pos_tag_prompt))]
        pos_caption_enc = torch.cat(pos_caption_enc, dim=0).to(lq_latent_hat.device)
    ...
    def _gaussian_weights(self, tile_width, tile_height, nbatches):
        ...
        return torch.tile(torch.tensor(weights), (nbatches, self.unet.config.out_channels, 1, 1))