"""
latent_codec_variable2_step_A3.py  —  A3 variant of latent codec.

A3 changes vs. A0 (latent_codec_variable2_step.py):
...
dual UncertaintyGateModule in StableCodec_variable2_step_A3.py to drive two
spatial gates: M_unet ∈ [0,1]^{B×1×H/8×W/8} for UNet repair residual and
M_aux ∈ [0,1]^{B×1×H/8×W/8} for AuxDecoder structural residual.
...
class LatentCodec(CompressionModel):
    ...
    def decompress(self, strings, shape, lmbda=None):
        ...
        # A3: scales_all must be returned for the model-level gate, so compute
