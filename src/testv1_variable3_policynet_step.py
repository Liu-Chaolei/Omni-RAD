"""
testv1_variable3_policynet_step.py — Testing script for variable-rate StableCodec
with learned TimestepPolicyNet.

Evaluates the model at multiple λ values to produce a full RD curve.
Uses StableCodec_variable2_step_policy (end-to-end PolicyNet).
Reports per-image and per-λ T_pred statistics alongside BPP/PSNR/LPIPS.
"""
...
def main(base_config, test_config, stage):
    ...
    for lmbda_val in lambda_list:
            ...
            with torch.no_grad():
                try:
                    ...
                    lmbda_tensor = torch.tensor([lmbda_val],
                                                dtype=torch.float32,
                                                device=device)
                    x_hat, RateLossOutput, T_pred, _ = net(img_padded, pos_tag_prompt,
                                                H, W, lmbda=lmbda_tensor)

                    bpp = RateLossOutput.quantized_total_bpp.item()
                    cur_tpred = T_pred.mean().item()
                    ...
                    tpred_meter.update(cur_tpred)
                    bpp_list.append(bpp)

                    print(f'  BPP: {bpp:.4f} | PSNR: {cur_psnr:.2f} | '
                          f'LPIPS: {cur_lpips:.4f} | T_pred: {cur_tpred:.1f}')
        ...
        avg_tpred = tpred_meter.avg
        all_results[lmbda_val] = {
            'bpp': avg_bpp, 'psnr': avg_psnr, 'lpips': avg_lpips, 't_pred': avg_tpred,
        }

        print(f'\n  [{tag}] Avg BPP: {avg_bpp:.4f} | '
              f'PSNR: {avg_psnr:.2f} | LPIPS: {avg_lpips:.4f} | T_pred: {avg_tpred:.1f}')
    ...
    for lmbda_val in lambda_list:
        ...
        if r:
            print(f'  {lmbda_val:10.3f}  {r["bpp"]:8.4f}  '
                  f'{r["psnr"]:8.2f}  {r["lpips"]:8.4f}  {r["t_pred"]:6.1f}')
    print(f'{"="*78}\n')
