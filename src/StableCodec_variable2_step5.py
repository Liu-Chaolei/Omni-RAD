"""
StableCodec_variable2_step5.py — StableCodec with variable-rate λ-FiLM conditioning,
SNR-based dynamic timestep T*, and curriculum learning support (t_override).

Changes from StableCodec_variable2_step.py:
  • forward() accepts t_override [B] tensor: when provided, overrides the codec's
    T_star for UNet and _batched_ddpm_step. The codec's natural T_star is still
...
class StableCodec(torch.nn.Module):
    ...
    def forward(self, x, pos_prompt, ori_h, ori_w, lmbda=None, t_override=None):
        """
        Args:
            ...
            t_override: Tensor [B] or None.  If provided, used as the UNet
                        timestep instead of the codec's natural T_star.
                        T_star is still returned for diagnostics.

        Returns:
            output_image  [B, 3, H, W]  in [-1, 1]
            RateLossOutput
            T_star        [B]            codec's natural dynamic timestep
        """
        B = x.shape[0]
        device = x.device
        ...
        # Use t_override (curriculum) if provided, else codec's natural T*
        t_for_unet = t_override if t_override is not None else T_star
        t_long = t_for_unet.long().to(device)
        model_pred = self.unet(
            lq_latent_hat, t_long,
        ...
        x_denoised = (
            self._batched_ddpm_step(model_pred, t_long,
                                    lq_latent_hat[:, :256])
            + res1
        )
    ...
    def decompress(self, strings, shape, pos_prompt, lmbda=None):
        ...
        lq_latent_hat, res, T_star = self.codec.decompress(strings, shape, lmbda)
